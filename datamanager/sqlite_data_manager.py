...
class User(db.Model):
    ...
    movies = db.relationship('User_Movie_Library', backref='user', lazy=True)
...
class Movie(db.Model):
    ...
    users = db.relationship('User_Movie_Library', backref='movie', lazy=True)

    def __str__(self):
        return f"Movie: {self.name} (ID: {self.id}, Director: {self.director}, Year: {self.year})"
